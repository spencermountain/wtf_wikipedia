...
test('toronto_star page', (t) => {
  ...
  t.equal(doc.infobox()._data.publisher.text(), 'John D. Cruickshank', 'publisher.text')
...
test('jodie_emery', (t) => {
  ...
  t.equal(doc.isRedirect(), false, 'not-redirect')
  t.equal(doc.infobox()._data.nationality.text(), 'Canadian', 'nationality')
...
test('statoil', (t) => {
  ...
  t.equal(doc.isRedirect(), false, 'not-redirect')
  t.equal(doc.infobox()._data.namn.text(), 'Statoil ASA', 'name')
...
test('raith rovers', (t) => {
  ...
  t.equal(doc.isRedirect(), false, 'not-redirect')
  t.equal(doc.infobox(0)._data.clubname.text(), 'Raith Rovers', 'clubname')
