...
test('complex-citation', (t) => {
  const str = `Emery is a vegetarian,<ref name="fun">{{ cite web|foo =    bar
...
test('multiple-citations', (t) => {
  const str = `hello {{citation |url=cool.com/?fun=yes/   }}{{CITE book |title=the killer and the cartoons }}`
  const arr = wtf(str)
    .citations()
    .map((c) => c.json())
...
test('weird-harvard-citations', (t) => {
  const str = `{{Harvnb|Selin|2008|p=}}{{cite web|url=https://www.thestar.com/news/city_hall/toronto2014election/2014/10/25/mayoral_candidate_john_tory_a_leader_from_childhood.html|title=Mayoral candidate John Tory a leader from childhood|newspaper=Toronto Star|date=October 25, 2014|first=Linda|last=Diebel|accessdate=October 28, 2014}}</ref>`
  const arr = wtf(str)
    .citations()
    .map((c) => c.json())
  t.equal(arr.length, 2, 'found-two-citations')
...
test('crazy-long-citations', (t) => {
  const str = `{{Begriffsklärungshinweis}}
...
test('inline-test', (t) => {
  const str = ` {{cite gnis|id=1145117|name=Little Butte Creek|entrydate=November 28, 1980|accessdate=September 26, 2009}}
