...
test('Parse DMS', function (t) {
  const str = `hi {{coord|59|12|7.7|N|02|15|39.6|W}} there`
  const obj = wtf(str).coordinate()
  const want = {
    lat: 59.20214,
    lon: -2.261,
  }
...
test('Parse DMS-missing', function (t) {
  const str = `hi {{coord|59|N|02|W}} there`
  const obj = wtf(str).coordinate()
  const want = {
    lat: 59,
    lon: -2,
