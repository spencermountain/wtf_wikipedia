...
test('royal_cinema', t => {
  ...
  str_equal(data.infoboxes[0].opened.text, 1939, t)
  str_equal(data.infoboxes[0].template, 'venue', t)
...
test('toronto_star', t => {
  ...
  str_equal(data.infoboxes[0].publisher.text, 'John D. Cruickshank', t)
  str_equal(data.infoboxes[0].template, 'newspaper', t)
...
test('jodie_emery', t => {
  ...
  str_equal(data.infoboxes[0].nationality.text, 'Canadian', t)
  str_equal(data.infoboxes[0].template, 'person', t)
...
test('redirect', t => {
  ...
  // str_equal(data.infoboxes[0], undefined, t)
  // str_equal(data.infoboxes[0].template, undefined, t)
  t.end()
})

test('statoil', t => {
  var data = wtf_wikipedia.parse(fetch('statoil'))
  str_equal(data.infoboxes[0].namn.text, 'Statoil ASA', t)
  str_equal(data.infoboxes[0].template, 'verksemd', t)
...
test('raith rovers', t => {
  ...
  str_equal(data.infoboxes[0].clubname.text, 'Raith Rovers', t)
