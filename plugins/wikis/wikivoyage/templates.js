...
const templates = {
  ...
  listing: (tmpl, list, parser) => {
    ...
    const out = `${name} ${obj.address || ''} ${obj.directions || ''} ${phone} ${obj.hours || ''} ${obj.content} ${obj.price} ${updated}`
    return out
  },
  // https://en.wikivoyage.org/wiki/Template:Station
  station: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['name'])
    list.push(obj)
  ...
  phone: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['number'])
    list.push(obj)
    return obj.number || ''
  },
  kilometer: (tmpl, _list, parser) => {
    const obj = parser(tmpl, ['km', 'adj'])
  ...
  confused: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['disambig'])
    list.push(obj)
    if (obj.disambig) {
  ...
  relatedwikipedia: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['page'])
    list.push(obj)
    return `[[Wikipedia:${obj.page}]]`
  },
  celsius: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['c'])
    list.push(obj)
    return `${obj.c}°C`
  },
  pound: (tmpl, list, parser) => {
    const obj = parser(tmpl, ['num'])
    list.push(obj)
    return `${obj.num} lb`
  },
